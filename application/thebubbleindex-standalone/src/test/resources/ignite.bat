...
if %ERRORLEVEL% equ 0 (
    if "%JVM_OPTS%" == "" set JVM_OPTS=-Xms1g -Xmx8g -server -XX:+AggressiveOpts -XX:MaxPermSize=256m
) else (
    if "%JVM_OPTS%" == "" set JVM_OPTS=-Xms1g -Xmx8g -server -XX:+AggressiveOpts -XX:MaxMetaspaceSize=256m
)

::
:: Uncomment the following GC settings if you see spikes in your throughput due to Garbage Collection.
::
::set JVM_OPTS=%JVM_OPTS% -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:+UseTLAB -XX:NewSize=128m -XX:MaxNewSize=128m -XX:+UseG1GC -XX:+DisableExplicitGC
set JVM_OPTS=%JVM_OPTS% -XX:+UseTLAB -XX:NewSize=128m -XX:MaxNewSize=128m -XX:+UseG1GC -XX:+DisableExplicitGC
