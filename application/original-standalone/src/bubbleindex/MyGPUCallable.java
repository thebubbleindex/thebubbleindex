...
/**
 * MyGPUCallable is the actual numerical calculation of The Bubble Index.
...
public class MyGPUCallable implements Callable<Float> {   
     ...
     */
    public MyGPUCallable (final BubbleIndexWorker bubbleIndexWorker, final int index,
            final int numberOfDays, final LombScargle lombScargle, 
            final CLContext context, final CLQueue queue, final CLProgram program, final CLKernel kernel, 
            final ByteOrder byteOrder, final double tCritDouble, final double[] dailyPriceValues,
     ...
     * <p>
     * The method sets up variables and then passes the final calculation
     * to the GPU device.
     * 
    ...
    public Float call() {
        ...
        if (!RunContext.Stop) {
            ...
            for (int i = 0; i < qSize; i++) {
                ...
                for (int p = 0; p < hSize; p++) {
                    ...
                    for (int k = 0; k < numberOfDays; k++) {
                        ...
                        final double tempValue = (FastMath.pow(TimeValues[k], 
                                mCoeffDouble - H[p])) *
            ...
            final CLBuffer<Float> 
                values = context.createFloatBuffer(CLMem.Usage.Output, Values),
