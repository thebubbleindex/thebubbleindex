...
/**
 * the noGUI class is the entry point of the application.
 * The Bubble Index can either run in a Graphical User
...
public class noGUI {
     ...
     * <p>
     * Accepts command line arguments. The order
     * of the arguments is important. "" indicates a string input.
     * <p>
     * <ol>
     * <li>Input either "noGUI" - runs in terminal, "GUI" - runs GUI JSwing</li>
     * <li>RunType Enumerator: input one of these strings: {"Single", "Category", "All", "Update"}</li>
     * <li>Category name: input the name of category. Example: "Currencies"</li>
     * <li>Windows: input the integer value of windows to run, separated by comma</li>
     * <li>Threads: input the integer value of threads. Recommend number of CPU threads.</li>
     ...
     * <li>Force CPU processing: input either "true" or "false". True will not run GPU contexts.</li>
     * </ol>
     * <p>
     * Command line example:
     * <p>
     * java -jar Bubble_Index.jar noGUI Category Currencies 700,800,900,1000,1500,2000,2200,2700 100 21.0 0.38 6.28 true
     * 
        ...
        if (args.length > 0) {
            ...
            if (args[i].equals("noGUI")) {
                ...
                if (type == RunType.Single) {
                    ...
                    Logs.myLogger.info("Running single selection. Category Name = {}, Selection Name = {}", categoryName, selectionName);
                    final String[] windowArray = windows.split(",");
                    
                    for (final String window : windowArray) {
                        try {
                            final BubbleIndex bubble = new BubbleIndex(omega, mCoeff, tCrit, Integer.parseInt(window.trim()), categoryName, selectionName);
                            bubble.runBubbleIndex(null);
                            bubble.outputResults(null);
                        } catch (final FailedToRunIndex ex) {
                            Logs.myLogger.error("Failed to Run Index. Category Name = {}, Selection Name = {}. {}", categoryName, selectionName, ex);
                        }
                    }
                }
